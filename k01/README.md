...
平均を求める関数、2乗の平均を求める関数、分散を求める関数を宣言
使用したいファイルを読み込む
そのファイルがNULLなら強制終了
ファイルを1行ずつbufに読み込む．もし読み込み成功する限り以下を繰り返す
bufからscanfで数値文字列を実数としてvalに読み込む
呼び出した関数をもとに、標本平均、2乗の平均、標本分散を求める
ファイルを閉じる．閉じれないときは，即座にプログラム終了
標本平均、標本分散、平均推定値、分散推定値を出力
ave_onlineを定義
